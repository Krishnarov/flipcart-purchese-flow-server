...
const failRecord = async (record, page, prefix, reason, io) => {
  ...
  try {
    ...
    console.error(`⚠️ [Automation] Error taking screenshot for ${record.email}: ${err.message}`);
  }
  ...
  console.log(`❌ [Automation] DB saved — ${record.email} - ${record.name} (Failed: ${reason})`);
};
...
const processLogin = async (records, emailContext, jobId, runHeadless, io) => {
  ...
  try {
    ...
    await flipkartPage.waitForLoadState('networkidle', { timeout: 3000 }).catch(() => { });
...
const processPurchase = async (records, jobId, runHeadless, io) => {
  ...
  const currentJobStatusBeforeAddress = (await AutomationJob.findById(jobId)).status;
  if (currentJobStatusBeforeAddress === 'stopped') {
    console.log(`🛑 [Automation] Job stopped before address processing for ${primaryRecord.email}.`);
    return;
  }

    if (isAddressSaved) {
      console.log(`[Automation] Addresses already saved for all tasks of ${primaryRecord.email}. Skipping address creation.`);
    } else {
      ...
      const addressHeader = flipkartPage.locator('text=/Manage Addresses/i').first();
      const anyAddressDot = flipkartPage.locator('.IVvp_M').first();
      const addAddrBtn = flipkartPage.locator('text=/ADD A NEW ADDRESS|ADD ADDRESSES/i').first();
      const loginForm = flipkartPage.locator('text=/Login/i').first();
      
      const loadedElem = addressHeader.or(anyAddressDot).or(addAddrBtn).or(loginForm);
      await loadedElem.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
      ...
      while (maxRetries > 0) {
        ...
        const jobStatusDuringDeletion = (await AutomationJob.findById(jobId)).status;
        if (jobStatusDuringDeletion === 'stopped') {
          console.log(`🛑 [Automation] Job stopped during address deletion for ${primaryRecord.email}.`);
          return;
        }
        maxRetries--;
        ...
        try {
          ...
          deletedCount++;
          console.log(`[Automation] Deleted address ${deletedCount} for ${primaryRecord.email}`);

          // Wait until the number of dots decreases in the DOM
          await Promise.all([
            flipkartPage.waitForFunction((expectedCount) => {
              return document.querySelectorAll('.IVvp_M').length < expectedCount;
            }, dotsCount, { timeout: 3000 }).catch(() => { }),
      ...
      // Now add new addresses
        ...
        if (jobStatusBeforeAdd === 'stopped') {
          console.log(`🛑 [Automation] Job stopped before adding new addresses for ${primaryRecord.email}.`);
          return;
        }
        ...
        try {
          ...
          const addBtn = addAddressesEmptyBtn.or(addNewAddressBtn).or(fallbackBtn);
          ...
          const jobStatusAfterAddress = (await AutomationJob.findById(jobId)).status;
          if (jobStatusAfterAddress === 'stopped') {
            console.log(`🛑 [Automation] Job stopped after adding address for ${r.email}.`);
            return;
          }
          await safeSaveRecord(r);
    ...
    for (const currentRecord of records) {
      ...
      if (!currentJob || currentJob.status === 'stopped') {
        if (currentRecord.status !== 'inprogress') {
          console.log(`🛑 [Automation] Job stopped by user. Halting remaining records for ${currentRecord.email}.`);
          break;
        }
      }
      ...
      const firstPlaceOrderBtn = flipkartPage.locator('text=/Place Order/i').first();
      const firstEmptyCartText = flipkartPage.locator('text="Your cart is empty!"').first();
      await firstPlaceOrderBtn.or(firstEmptyCartText).waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
      ...
      while (cartMaxRetries > 0) {
        ...
        if (await removeBtn.isVisible().catch(() => false)) {
          ...
          await confirmRemoveBtn.waitFor({ state: 'visible', timeout: 2000 }).catch(() => {});
          await confirmRemoveBtn.click({ timeout: 1500 }).catch(() => {});
          
          // Wait for the remove button to be detached
          await removeBtn.waitFor({ state: 'detached', timeout: 3500 }).catch(() => {});
      ...
      const outOfStockText = flipkartPage.locator('text=/Out of stock|Sold Out|Currently unavailable/i').first();
      ...
      const isUnavailable = await unavailableLocator.waitFor({ state: 'visible', timeout: 1000 }).then(() => true).catch(() => false);

      if (isUnavailable) {
        const isNotifyMe = await notifyMeBtn.isVisible().catch(() => false);
        const failureReason = isNotifyMe ? 'Notify Me' : 'Out of Stock';
        await failRecord(currentRecord, flipkartPage, 'product-failed', failureReason, io);
        continue;
      }
      ...
      if (clickedAddToCart) {
        ...
        await Promise.race([
          flipkartPage.waitForURL(url => url.includes('/viewcart'), { timeout: 3500 }).catch(() => {}),
          goToCartBtn.waitFor({ state: 'visible', timeout: 3500 }).catch(() => {})
        ]);

        await flipkartPage.goto('https://www.flipkart.com/viewcart', { waitUntil: 'domcontentloaded' });
        
        const placeOrderBtn = flipkartPage.locator('text=/Place Order/i').first();
        const emptyCartText = flipkartPage.locator('text="Your cart is empty!"').first();
        await placeOrderBtn.or(emptyCartText).waitFor({ state: 'visible', timeout: 6000 }).catch(() => {});
        ...
        if (isBtnVisible) {
          ...
          const nameSelector = flipkartPage.locator('div').filter({ hasText: new RegExp(currentRecord.name, 'i') }).first();
          await nameSelector.waitFor({ state: 'visible', timeout: 3500 }).catch(() => {});
        } else {
        ...
        if (currentRecord.name) {
          ...
          if (clickedAddress) {
            ...
            console.log(`[Automation Tracking] Waiting for 'Deliver Here' to disappear and cart to update...`);
            await deliverHereBtn.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
        ...
        try {
          ...
          await continueBtn.waitFor({ state: 'visible', timeout: 6000 }).catch(() => {});

          await emitLiveLog(currentRecord, 'Clicking Continue...', 'inprogress', io);
          await continueBtn.click({ timeout: 5000 });
          
          // Wait dynamically for Payment options (Cash on Delivery or others) to show
          const codOption = flipkartPage.locator('text=/Cash on Delivery/i').first();
          const otherPaymentOption = flipkartPage.locator('text=/Net Banking|Credit.*Card/i').first();
          await codOption.or(otherPaymentOption).waitFor({ state: 'visible', timeout: 6000 }).catch(() => {});
          ...
          await codPlaceOrderBtn.waitFor({ state: 'visible', timeout: 3000 }).catch(() => {});
          ...
          await confirmOrderBtn.waitFor({ state: 'visible', timeout: 4000 }).catch(() => {});

          await emitLiveLog(currentRecord, 'Confirming order...', 'inprogress', io);
          await confirmOrderBtn.click({ timeout: 5000 });
          
          // Wait dynamically for success page url or message
          await Promise.race([
            flipkartPage.waitForURL(url => url.includes('/success') || url.includes('/order'), { timeout: 8000 }).catch(() => {}),
            flipkartPage.locator('text=/Order Placed.*Successfully|Thank you/i').waitFor({ state: 'visible', timeout: 8000 }).catch(() => {})
          ]);

          // Dynamically wait for the success URL to contain reference_id (up to 15s)
          let orderId = '';
          try {
            await flipkartPage.waitForURL(
              url => url.includes('reference_id'),
              { timeout: 15000 }
            );
            ...
            console.warn(`⚠️ [Automation] Could not get order ID from URL: ${e.message}`);
            // Fallback: try current URL anyway
            try {
              const fallbackUrl = flipkartPage.url();
              const fallbackObj = new URL(fallbackUrl);
              const fallbackRef = fallbackObj.searchParams.get('reference_id');
              if (fallbackRef) orderId = `${fallbackRef}00`;
            } catch (_) {}
          }
          ...
          if (orderId) currentRecord.orderId = orderId;
          await emitLiveLog(currentRecord, `Successfully confirmed COD order.${orderId ? ` Order ID: ${orderId}` : ''}`, 'success', io);
          ...
          await failRecord(currentRecord, flipkartPage, 'product-failed', cleanMsg, io);
        }
      } else if (await notifyMeBtn.isVisible().catch(() => false)) {
        await failRecord(currentRecord, flipkartPage, 'product-failed', 'Notify Me', io);
      } else {
        await failRecord(currentRecord, flipkartPage, 'product-failed', 'Add to cart button not found', io);
      }
    }
...
export const runFlipkartAutomation = async (jobId, userId, runHeadless = true, io = null, targetRecordId = null) => {
  ...
  if (!isPurchase) {
    ...
    try {
      ...
      if (io) {
        io.emit('job-update', { jobId, type: 'status-change', status: 'failed' });
      }
      return;
    }
  }
  ...
  if (targetRecordId) {
    resetQuery._id = Array.isArray(targetRecordId) ? { $in: targetRecordId } : targetRecordId;
  }
  ...
  while (hasMoreToProcess) {
    ...
    if (targetRecordId) {
      query._id = Array.isArray(targetRecordId) ? { $in: targetRecordId } : targetRecordId;
    }
    ...
    if (io) {
      io.emit('job-update', { jobId, type: 'status-change', status: 'running' });
    }
  ...
  if (finalJob) {
    ...
    const cleanupResult = await Purchase.updateMany(
      ...
      { status: 'pending', reason: 'Stopped by user before completion', screenshot: '' }
    );
    ...
    if (io) {
      io.emit('job-update', { jobId, type: 'status-change', status: finalJob.status });
